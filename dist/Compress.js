...
function compressionSearch(buffer, bufferPosition, bufferMaxLength, bufferLength, searchRange) {
    let foundLength = 1;
    let found = 0;
    if (!searchRange) {
...
function compressBuffer(buffer, level) {
    let searchRange;
    if (!level) {
        searchRange = 0;
    }
    ...
    while (bufferPosition < bufferLength) {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            const { found, foundLength } = compressionSearch(buffer, bufferPosition, maxLen, bufferLength, searchRange);
            if (foundLength > 2) {
                const delta = bufferPosition - found - 1;
                if (foundLength < 0x12) {
                    resultArray.push((delta >> 8) | ((foundLength - 2) << 4), delta & 0xff);
                }
                else {
                    resultArray.push(delta >> 8, delta & 0xff, (foundLength - 0x12) & 0xff);
                }
